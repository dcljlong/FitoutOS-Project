...
# ============== RESTORED MODEL DEFINITIONS FROM DEDUPLICATED PREFIX ==============
...
async def analyze_job_files(job_id: str, user: dict = Depends(require_roles(UserRole.ADMIN, UserRole.PM))):
    ...
    for line in raw_extracted_lines:
        ...
        target_is_fitout_contract = (
            ...
            or "craigs" in job_context_text
            or current_contract_hint == "fitout"
        )

        basebuild_ownership_terms = [
            "part of basebuild",
            "apart of basebuild",
            "part of bsebuild",
            "apart of bsebuild",
...
@api_router.post("/jobs/{job_id}/programme", response_model=ProgrammeItemResponse)
async def create_programme_item(job_id: str, item_data: ProgrammeItemCreate, user: dict = Depends(require_roles(UserRole.ADMIN, UserRole.PM))):
    """Add a single programme item to a job."""
    job = await db.jobs.find_one({"id": job_id})
    if not job:
        raise HTTPException(status_code=404, detail="Job not found")
    ...
    await db.job_programme.insert_one(item)
    return ProgrammeItemResponse(**item)


@api_router.put("/jobs/{job_id}/programme/{item_id}", response_model=ProgrammeItemResponse)
async def update_programme_item(job_id: str, item_id: str, item_data: ProgrammeItemCreate, user: dict = Depends(require_roles(UserRole.ADMIN, UserRole.PM))):
    """Update a programme item."""
    existing = await db.job_programme.find_one({"id": item_id, "job_id": job_id})
    if not existing:
        raise HTTPException(status_code=404, detail="Programme item not found")

    update_data = item_data.model_dump()
    await db.job_programme.update_one({"id": item_id}, {"$set": update_data})
    updated = await db.job_programme.find_one({"id": item_id}, {"_id": 0})
    return ProgrammeItemResponse(**updated)


@api_router.delete("/jobs/{job_id}/programme/{item_id}")
async def delete_programme_item(job_id: str, item_id: str, user: dict = Depends(require_roles(UserRole.ADMIN, UserRole.PM))):
    """Delete a programme item."""
    result = await db.job_programme.delete_one({"id": item_id, "job_id": job_id})
    if result.deleted_count == 0:
        raise HTTPException(status_code=404, detail="Programme item not found")
    return {"message": "Programme item deleted"}


class ProgrammeBulkSave(BaseModel):
    items: List[Dict[str, Any]]


@api_router.post("/jobs/{job_id}/programme/bulk-save")
async def bulk_save_programme(job_id: str, data: ProgrammeBulkSave, user: dict = Depends(require_roles(UserRole.ADMIN, UserRole.PM))):
    """Save entire programme (replace all items). Used for confirm workflow."""
    job = await db.jobs.find_one({"id": job_id})
    if not job:
        raise HTTPException(status_code=404, detail="Job not found")

    # Delete existing programme items
    await db.job_programme.delete_many({"job_id": job_id})

    # Insert new items
    saved_items = []
    for idx, item in enumerate(data.items):
        item_id = item.get("id") or f"prog-{idx+1:03d}"
        programme_item = {
            "id": item_id,
            "job_id": job_id,
            "name": item.get("name", ""),
            ...
            "task_code_id": item.get("task_code_id"),
            "crew_size": item.get("crew_size"),
    ...
    return {
        "message": f"Saved {len(saved_items)} programme items",
        "count": len(saved_items),
        "items": [{k: v for k, v in item.items() if k != "_id"} for item in saved_items]
    }


@api_router.post("/jobs/{job_id}/confirm-parsed-programme")
async def confirm_parsed_programme(
    job_id: str,
    data: ProgrammeBulkSave,
    user: dict = Depends(require_roles(UserRole.ADMIN, UserRole.PM))
):
    """
    Confirm edited parsed programme items and save them as the official job programme.
    This replaces any existing programme items.
    """
    job = await db.jobs.find_one({"id": job_id})
    if not job:
        raise HTTPException(status_code=404, detail="Job not found")
    ...
    # Mark parsed programme as confirmed
    await db.job_programme_parsed.update_one(
        {"job_id": job_id},
        {"$set": {"status": "confirmed", "confirmed_at": datetime.now(timezone.utc).isoformat()}}
    )

    return {
        "message": f"Confirmed {len(saved_items)} programme items",
        "count": len(saved_items),
        "items": [{k: v for k, v in item.items() if k != "_id"} for item in saved_items]
    }
