...
export default function JobDetailPage() {
  ...
  const fetchJobData = useCallback(async () => {
    try {
      const [jobRes, tasksRes, codesRes, subsRes, delaysRes, programmeRes] = await Promise.all([
        api.get(`/jobs/${job_id}`),
        api.get(`/tasks?job_id=${job_id}`),
        api.get(`/jobs/${job_id}/task-codes`),
        api.get('/subcontractors'),
        api.get(`/delays?job_id=${job_id}`),
        api.get(`/jobs/${job_id}/programme`),
      ]);
      setJob(jobRes.data);
      setTasks(tasksRes.data?.value || []);
  ...
  const handleGenerateTasksFromProgramme = async () => {
    try {
      const response = await api.post(`/jobs/${job_id}/programme/generate-tasks`);
        ...
        <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
          ...
          {canManage() && (
            <div className="flex gap-2 flex-wrap">
              <Link to={`/jobs/${job_id}/gantt`}>
                <Button variant="outline">
                  View Gantt
                </Button>
              </Link>
              <Link to={`/jobs/${job_id}/resources`}>
                <Button variant="outline">
                  Resource Analysis
                </Button>
              </Link>
              <Link to={`/jobs/${job_id}/setup`}>
                <Button variant="outline">
                  <Upload className="mr-2 h-4 w-4" />
                  Upload & Analyze
                </Button>
              </Link>
              <Button variant="outline" onClick={() => window.location.assign(`/jobs/${job_id}/setup?step=0`)}>
